locals {
  ...
  api_gateway_stage_invoke_fqdn = element(split("/", aws_api_gateway_stage.main.invoke_url), 2)
}
...
resource "aws_api_gateway_base_path_mapping" "main" {
  ...
  domain_name = var.enable_custom_cname_creation ? aws_api_gateway_domain_name.main[0].domain_name : "${var.custom_cname_hostname}.${var.custom_cname_domain}"
